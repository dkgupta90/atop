...
/*		Vector<float> estimated_error_per_cell(triangulation.n_active_cells());
	...
	double TopOptimization<dim>::get_penal_power(double penal_power,
		...
		double dpenal = penal_power - start_penal;
...
;
		...
		if(itr_count == 0){
			...
			myfile1<<"Cycle : "<<cycle+1<<"\\qquad"<<" No. of elem : "<<triangulation.n_active_cells()<<"\\qquad ";
			myfile1<<"$r_{min}$ : "<<max_projection_radius<<"\\qquad \n";
			myfile1.close();
		}
		boundary_info();
		setup_system();
...
;
	...
	void TopOptimization<dim>::run(){
		...
		for(; cycle < no_cycles; ++cycle){
			...
			std::ofstream myfile1;
			myfile1.open(output_fname, std::ofstream::app);
			myfile1<<"Iterations : "<<itr_count<<"\\qquad"<<" Objective : "<<objective<<"\\qquad \\\\ \n";
...
int main()
{
	try{
		...
		topopt2d.no_iterations = 25;
		topopt2d.max_projection_radius = 0.016;
		topopt2d.gamma = 1.0;
		topopt2d.cycle = 0;
		topopt2d.no_cycles = 4;
