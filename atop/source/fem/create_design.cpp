...
void CreateDesign<dim>::assemble_design(
	...
	for (typename hp::DoFHandler<dim>::active_cell_iterator cell = fem->design_handler.begin_active();
	...
	std::cout<<"No of design elements : "<<design_info_vector.size()<<std::endl;
	...
	for (unsigned int cell_itr = 0; cell_itr < fem->cell_info_vector->size(); ++cell_itr){
		...
		for(unsigned int j = 0; j < (*(fem->cell_info_vector))[cell_itr].pseudo_design_points.no_points; j++){
			//Creating the point
			Point<dim> pointX;
				for (unsigned int k = 0; k < dim; k++){
					pointX(k) = (*(fem->cell_info_vector))[cell_itr].pseudo_design_points.pointX[j][k];	//in local coordinates
			}
			...
			design_info_vector[cell_index].cell_density = (*(fem->cell_info_vector))[cell_itr].pseudo_design_points.rho[j];
			design_info_vector[cell_index].projection_radius = (*(fem->cell_info_vector))[cell_itr].projection_radius;
			design_info_vector[cell_index].cell_area_fraction =  (*(fem->cell_info_vector))[cell_itr].cell_area_fraction;
		}
		++cell;
	}
	...
	for (design_itr = 0; design_itr < design_info_vector.size(); ++design_itr){
		...
		design_info_vector[design_itr].cell_area_fraction =  (*(fem->cell_info_vector))[cell_itr].cell_area_fraction;
...
/*	design_cell = fem->design_handler.begin_active();
	...
	out_soln.write_density(design_info_vector,
			fem->cycle,
			fem->itr_count);
