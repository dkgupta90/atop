...
/*
template <int dim>
double QRIndicator<dim>::get_Jvalue(hp::DoFHandler<dim>::active_cell_iterator cell,
		Vector<double> &u_solution,
		Vector<double> &f_solution,
		unsigned int new_p){
...
*/
 ...
 //This function checks the J/Jstar values and proposed the p-order to be used for each finite element
template <int dim>
void QRIndicator<dim>::estimate_3D(std::vector<double> &qr_error){
	...
	for (; cell != endc; ++cell){
		...
		while (new_p >= (*cell_info_vector)[cell_itr].old_shape_fn_order + 1){
			// Get the Jvalue for this value of p
			double Jstar = get_Jvalue_3D(cell, u_solution, f_solution, new_p);
			sum_JJstar += (Jvalue/Jstar);
			//std::cout<<cell_itr<<"  "<<new_p<<"  "<<Jvalue<<"  "<<Jstar<<"  "<<Jstar/Jvalue<<std::endl;
			new_p-=1;
		}
 ...
 */
template <int dim>
double QRIndicator<dim>::get_Jvalue_3D(hp::DoFHandler<3>::active_cell_iterator cell,
		Vector<double> &u_solution,
		Vector<double> &f_solution,
		unsigned int new_p){
...
/*		(*fem).density_field.find_neighbors(cell,
		...
		(*fem).density_field.find_neighbors_3D(cell,
						new_cell,
						fe_values,
						new_cell_info,
						*(cell_info_vector));
