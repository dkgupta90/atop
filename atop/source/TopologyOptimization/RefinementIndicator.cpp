...
void DensityIndicator::get_density_indicator(
	...
	std::string RItype = "sens_dens_based";
...
/*
	...
	else if(RItype == "sens_dens_based"){

		std::vector<unsigned int> sel_cells;
		std::vector<double> max_sens;
		sel_cells.clear();
		max_sens.clear();
/*
		double rhomin = 0.0;
		...
		for(; cell != endc; ++cell, ++density_cell){
			...
			double total_weight = 0.0;
			double density = 0.0;
			cellprop[cell_itr].sens_contrast = 9999;
			unsigned int quad_index = elastic_data.get_quad_index(cellprop[cell_itr].quadrature_formula);
			for(unsigned int k = 0; k < elastic_data.JxW[quad_index].size(); ++k){
				total_weight += elastic_data.JxW[quad_index][k];
			}
			...
			if(density >= coarsen_ubound || density <= coarsen_lbound){
				...
				sel_cells.push_back(cell_itr);

				auto sens_low = std::min(cellprop[cell_itr].dcdx.begin(), cellprop[cell_itr].dcdx.end());
				auto sens_high = std::max(cellprop[cell_itr].dcdx.begin(),cellprop[cell_itr].dcdx.end());
				double sens_contr = fabs(cellprop[cell_itr].dcdx[*sens_high] - cellprop[cell_itr].dcdx[*sens_low]);
				//std::cout<<sens_contr<<std::endl;
				max_sens.push_back(sens_contr);
				cellprop[cell_itr].sens_contrast = sens_contr;

			}
			++cell_itr;
		}
		std::sort(max_sens.begin(), max_sens.end());

		unsigned int temp_len = max_sens.size();
		double cutoff_sens = max_sens[floor(temp_len*0.1)];
		//std::cout<<cutoff_sens<<std::endl;
		//Selecting the cells for refinement
		cell_itr = 0;
		cell = triangulation.begin_active();
		density_cell = density_triangulation.begin_active();
		endc = triangulation.end();
		density_endc = density_triangulation.end();

		for(; cell != endc; ++cell, ++density_cell){
			if((cellprop[cell_itr].sens_contrast - cutoff_sens) > 1e-16){
				cell_itr++;
				continue;
			}
			cell->set_refine_flag();
